...
run:
		$(GOBUILD) -o $(BINARY_NAME) -v cmd/cli/main.go
		./$(BINARY_NAME) --config configs/config.yml

build:
		$(GOBUILD) -o $(BINARY_PATH)$(BINARY_NAME) -v cmd/cli/main.go

build-all:
		GOOS=linux GOARCH=amd64 $(GOBUILD) -o $(BINARY_PATH)$(BINARY_LINUX) -v cmd/cli/main.go
		GOOS=freebsd GOARCH=amd64 $(GOBUILD) -o $(BINARY_PATH)$(BINARY_FREEBSD) -v cmd/cli/main.go
